...
export const ModelImporter: React.FC = () => {
  ...
  const listModels = async () => {
    ...
    try {
      ...
      setFiles(ggufFiles);
      setMessage(ggufFiles.length ? `Found ${ggufFiles.length} model(s) in app storage.` : 'No models found. Please copy a .gguf file to the app\'s internal models folder (use a file manager).');
    } catch (err: any) {
      console.error(err);
